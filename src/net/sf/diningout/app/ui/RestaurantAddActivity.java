...
public class RestaurantAddActivity extends SprocketsActivity implements LoaderCallbacks<Cursor>,
		...
		case R.id.done:
			...
			if (mId != null) { // check for existing Google restaurant, update or insert new
				...
				if (id <= 0) { // insert new
					...
					cr.update(ContentUris.withAppendedId(Restaurants.CONTENT_URI, id), vals, null,
							null);
				}
				c.close();
				if (id > 0) {
					startService(new Intent(this, RestaurantService.class).putExtra(EXTRA_ID, id));
				}
	...
	public void onRestaurantNameChanged(CharSequence name) {
		...
		if (name.length() > 0) {
			...
			if (view.getCheckedItemCount() > 0) { // switch from list to autocomplete
				view.setItemChecked(view.getCheckedItemPosition(), false);
				reset();
			}
		} else {
			setTitle(R.string.add_restaurant_title);
			if (view.getCheckedItemCount() == 0) { // forget any autocompletion
				reset();
			}
		}
	}
	 ...
	 */
	private void fields(Place place) {
		mId = place.getId();
	...
	private RestaurantAutocompleteFragment autocomplete() {
		return (RestaurantAutocompleteFragment) getFragmentManager().findFragmentById(
				R.id.autocomplete);
	}
