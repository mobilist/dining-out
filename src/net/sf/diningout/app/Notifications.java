...
public class Notifications {
	 ...
	 * 
	 * @param id
	 *            one of the ID_* constants in this class
	 * @param number
	 *            total number of items, may be more than number of lines
	 */
	public static void inboxStyle(int id, CharSequence title, Iterable<CharSequence> lines,
		...
		TaskStackBuilder task = TaskStackBuilder.create(context).addNextIntentWithParentStack(
				activity.addFlags(FLAG_ACTIVITY_NEW_TASK));
		PendingIntent content;
		if (id == ID_SYNC) { // remove after Android issue 41253 is fixed
			Intent read = new Intent(context, SyncsReadService.class).putExtra(EXTRA_ACTIVITIES,
					task.getIntents());
		...
		Builder notif = new Builder(context).setDefaults(defaults).setOnlyAlertOnce(true)
				.setTicker(title).setContentTitle(title).setWhen(when).setLargeIcon(icon)
				.setSmallIcon(R.drawable.stat_logo).setNumber(number).setContentIntent(content)
				.setAutoCancel(true);
