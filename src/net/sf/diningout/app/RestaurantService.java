...
public class RestaurantService extends IntentService {
	...
	public static void download(long id) {
		...
		if (restaurant != null && !TextUtils.isEmpty(restaurant.googleReference)) {
			...
			if (details.first != null && details.second >= 0) {
				try {
	 ...
	 * 
	 * @param vals
	 *            should have a size of 15 or greater and must include
	 *            {@link Restaurants#GOOGLE_REFERENCE}
	 * @return {@link Places#details(Params, Field...) details} response and ID of the first photo,
	 *         0 if there aren't any photos, or -1 if we already have photos
	 */
	static Pair<Place, Long> details(long id, ContentValues vals) {
		...
		return Pair.create(place, photoId);
	}
	 ...
	 * 
	 * @param id
	 *            can be 0 if the place doesn't have any photos and a Street View image should be
	 *            downloaded
	 */
