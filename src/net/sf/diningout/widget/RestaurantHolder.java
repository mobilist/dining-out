...
public class RestaurantHolder extends ViewHolder {
	 ...
	 * 
	 * @param millis
	 *            0 if the restaurant has never been visited
	 */
	RestaurantHolder visit(long millis) {
		if (millis > 0) {
			long now = System.currentTimeMillis();
			detail.setText(now - millis > MINUTE_IN_MILLIS ? DateUtils.getRelativeTimeSpanString(
					millis, now, 0, FORMAT_ABBREV_ALL) : mContext.getString(R.string.recent_time));
		} else {
